...
export class FirestoreStorage implements IStorage {
  ...
  private imagesCol = firestore.collection("images");
  ...
  async saveImage(data: string): Promise<string> {
    const docRef = await this.imagesCol.add({ data, createdAt: new Date().toISOString() });
    return docRef.id;
  }

  async getImage(id: string): Promise<string | null> {
    const doc = await this.imagesCol.doc(id).get();
    if (!doc.exists) return null;
